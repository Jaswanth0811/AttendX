...
class UpdateService {
  Future<void> checkForUpdates(BuildContext context, {bool silent = false}) async {
    try {
      ...
      if (isNewer) {
        if (context.mounted) {
          String downloadUrl = data['html_url']?.toString() ?? '';
          ...
          if (assets != null && assets.isNotEmpty) {
            ...
            if (apkAsset != null) {
              downloadUrl = apkAsset['browser_download_url']?.toString() ?? downloadUrl;
            }
          }
          _showUpdateDialog(context, latestTag, data['body']?.toString() ?? '', downloadUrl);
        }
      } else {
